...
public class PyVenvRunner : IDisposable, IAsyncDisposable
{
    ...
    /// </summary>
    public static string RelativeBinPath => Compat.Switch((PlatformKind.Windows, "Scripts"), (PlatformKind.Unix, "bin"));
    ...
    )
    {
        ...
        AnsiProcess venvProc = ProcessRunner.StartAnsiProcess(
    ...
    )
    {
        ...
        if (Compat.IsWindows)
        {
            DirectoryPath portableGitBin = GlobalConfig.LibraryDir.JoinDir("PortableGit", "bin");
            DirectoryPath venvBin = RootPath.JoinDir(RelativeBinPath);
        ...
        if (onExit != null)
        {
            ...
            Process.Exited += (sender, _) => { onExit((sender as AnsiProcess)?.ExitCode ?? -1); };
        }
    }
    ...
    public async Task<string?> GetEntryPoint(string entryPointName)
    {
        ...
        ProcessResult result = await Run($"-c \"{code}\"").ConfigureAwait(false);
