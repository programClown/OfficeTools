...
public static class FileTransfers
{
    ...
    )
    {
        ...
        foreach (DirectoryInfo subDir in sourceDir.Info.EnumerateDirectories())
        {
            DirectoryPath destinationSubDir = destinationDir.JoinDir(subDir.Name);
    ...
    {
        foreach (FileInfo file in sourceDir.Info.EnumerateFiles())
        {
            FilePath sourceFile = sourceDir.JoinFile(file.Name);
            FilePath destinationFile = destinationDir.JoinFile(file.Name);
