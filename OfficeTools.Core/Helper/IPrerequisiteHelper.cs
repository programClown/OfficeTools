...
using OfficeTools.Core.Packages;
...
public interface IPrerequisiteHelper
{
    ...
    {
        ProcessResult result = await GetGitOutput(["rev-parse", "--is-inside-work-tree"], repositoryPath)
    ...
    Task<ProcessResult> GetGitRepositoryRemoteOriginUrl(string repositoryPath) =>
        GetGitOutput(["config", "--get", "remote.origin.url"], repositoryPath);
