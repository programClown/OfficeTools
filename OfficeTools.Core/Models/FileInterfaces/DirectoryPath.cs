...
public class DirectoryPath : FileSystemPath, IPathObject, IEnumerable<FileSystemPath>
{
    ...
    public DirectoryPath RelativeTo(DirectoryPath path) => new(Path.GetRelativePath(path.FullPath, FullPath));
    ...
    public void CopyTo(DirectoryPath destinationDir, bool recursive = true)
    {
        ...
        foreach (FilePath file in EnumerateFiles())
        {
            FilePath targetFilePath = destinationDir.JoinFile(file.Name);
        ...
        if (recursive)
        {
            ...
            {
                DirectoryPath targetDirectory = destinationDir.JoinDir(subDir.Name);
    ...
    public static string operator +(DirectoryPath path, [Localizable(false)] string other) => Path.Combine(path, other);
