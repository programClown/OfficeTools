...
public partial class FilePath : FileSystemPath, IPathObject
{
    ...
    public FilePath RelativeTo(DirectoryPath path) => new(Path.GetRelativePath(path.FullPath, FullPath));
    ...
    public Task<string> ReadAllTextAsync(CancellationToken ct = default) => File.ReadAllTextAsync(FullPath, ct);
    ...
    public Task WriteAllTextAsync(string text, CancellationToken ct = default, Encoding? encoding = null) =>
        File.WriteAllTextAsync(FullPath, text, encoding ?? new UTF8Encoding(false), ct);

    /// <summary> Read bytes </summary>
    public byte[] ReadAllBytes() => File.ReadAllBytes(FullPath);

    /// <summary> Read bytes asynchronously </summary>
    public Task<byte[]> ReadAllBytesAsync(CancellationToken ct = default) => File.ReadAllBytesAsync(FullPath, ct);
    ...
    public Task WriteAllBytesAsync(byte[] bytes, CancellationToken ct = default) =>
        File.WriteAllBytesAsync(FullPath, bytes, ct);
    ...
    public async Task<FilePath> MoveToWithIncrementAsync(FilePath destinationFile, int maxTries = 100)
    {
        ...
        FilePath targetFile = destinationFile;
    ...
    {
        await using FileStream sourceStream = Info.OpenRead();
        await using FileStream destinationStream = destinationFile.Info.OpenWrite();
